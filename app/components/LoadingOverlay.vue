...
<template>
	<Transition name="fade">
		<div v-if="loaderStore.isLoading"
			...
			@click.stop.prevent @touchmove.prevent>
			<!-- Background Effects -->
			<div class="absolute inset-0 overflow-hidden pointer-events-none">
				<div
					class="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-violet-600/20 rounded-full blur-[120px] animate-pulse">
				</div>
				<div
					class="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] bg-fuchsia-600/20 rounded-full blur-[120px] animate-pulse"
			...
			<div v-if="loaderStore.showProgress"
				class="w-full max-w-xl space-y-6 relative z-10 bg-white/5 p-8 rounded-3xl border border-white/10 backdrop-blur-md shadow-2xl">
				<div class="flex justify-between items-end text-white font-bold">
					<span class="text-lg text-white/80">Carregando biblioteca...</span>
					<span
						class="text-4xl tabular-nums text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400">{{
							percentage }}%</span>
				</div>
